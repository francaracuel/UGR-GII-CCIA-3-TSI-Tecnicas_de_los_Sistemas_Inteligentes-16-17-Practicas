...
void FrontierExplorer::gira360(){
    ...
    // Yaw contiene la orientación del robot. Cuando gire 360º volverá a la
    // misma posición.
    // Cuando esté iniciando el giro, el contador tiene 0. Así se detecta que
    // va a comenzar.
    /*if(!isAction){
        aux = yaw;
        isAction = !isAction;
    ...
    // Se comprueba si el yaw con el que comenzó se encuentra en un intervalo,
    // entre el actual y el anterior
    if((vel.angular.z>0 && oldYaw<aux && aux<=yaw)){
        iAction++;
    }
    
    
    
    // Si aún sigue realizando el giro
    if(iAction < iActionStop){
        
        // Se gira en el eje Z a una velocidad dada
        msg.angular.z = V_ANGULAR_CTE;
        
    // Cuando ya ha terminado de hacer el giro, se resetean los valores
    }else{
...
void FrontierExplorer::processActions(){
    ...
    cout << "OldYaw: " << oldYaw << endl;
    
    cout << "Aux: " << aux << endl;
    
    cout << "Action: " << action << endl;
    
    cout << "iAction: " << iAction << endl;
    
    cout << "iActionStop: " << iActionStop << endl;
    ...
    // Se añaden los puntos que están en la frontera
    //labelFrontierNodes();
...
void FrontierExplorer::selectNearestNode(nodeOfFrontier &selectedNode){
    ...
    int difference = getDifferenceAbsNodes(nodoPosicionRobot, frontera[posNode]);
    ...
    for(unsigned int i=1; i<frontera.size(); i++){
        ...
        int differenceAux = getDifferenceAbsNodes(nodoPosicionRobot, frontera[i]);
        
        
        //////////////////////////////////////////////////////////////////////
        // Puede que se quede dando vueltas. Se puede poner una distancia minima
        ///////////////////////////////////////////////////////////////////////
        
        // Si la diferencia es menor se actualiza
        if(differenceAux < difference){
