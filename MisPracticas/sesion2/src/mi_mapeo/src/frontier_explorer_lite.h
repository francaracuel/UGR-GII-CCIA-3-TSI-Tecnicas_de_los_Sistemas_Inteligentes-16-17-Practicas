...
class FrontierExplorer
{
    public:
        ...
        nodeOfFrontier nodoPosicionRobot; //Guarda la posicion del robot capturada en la callback de amcl
        nodeOfFrontier nodoObjetivo; //Posición del objetivo del robot
        TipoFrontera frontera; //Lista de las posiciones de los puntos de la frontera detectada

        geometry_msgs::PoseWithCovarianceStamped odometria;      //guarda el último mensaje de odometría recibido desde amcl
        nav_msgs::OccupancyGrid cmGlobal; //Guarda el último mensaje de mapa recibido desde gmapping

        std::vector<std::vector<int> > theGlobalCm; // Matriz que alamacena el mapa, todas las operaciones sobre mapa van referidas a esta matriz
    ...
    private:
        ...
        void odomCallBack(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr& msg); //Callback para obtener la pose estimada publicada por amcl
        void getmapCallBack(const nav_msgs::OccupancyGrid::ConstPtr &msg);//Callback para obtener el mapa publicado por gmapping
