...
(define (domain ej5-domain)
    ...
    (:functions
        ...
        ; Indica los puntos que obtiene el jugador por entregar un objeto a
        ; un personaje
        (pointsGiving ?c - character ?o - obj)

        ; Se debe almacenar los puntos acumulados del jugador
        (totalPoints)

        ; Se debe llevar la cuenta de la cantidad de objetos que hay en la
        ; mochila
        (countBackpack)

        ; También se tiene que saber cuál es la capacidad máxima que tiene la
        ; mochila
        (backpackCapacity)
    ...
    (:action PUT_BACKPACK
        ...
        ; vacía.
        ; El jugador no puede tener el mismo objeto en la mochila.
        ; La mochila no puede estar llena
        :precondition (and
            (not (EH))
            (has ?p ?o)
            (not (backpack ?o))
            (< (countBackpack) (backpackCapacity))
        )

        ; Efectos que produce la acción:
        ; El jugador tiene la mano vacía y el objeto está en la mochila.
        ; La mochila tiene un objeto mas.
        :effect (and
            (EH)
            (backpack ?o)
            (increase (countBackpack) 1)
        )
    ...
    (:action POP_BACKPACK
        ...
        ; La mochila tiene un objeto menos.
        :effect (and
            (not (EH))
            (not (backpack ?o))
            (decrease (countBackpack) 1)
        )
    ...
    (:action GIVE
        ...
        :effect (and
            ...
            (EH)
            (has ?c ?o)
            (increase (totalPoints) (pointsGiving ?c ?o))
        )
